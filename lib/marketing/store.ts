...
export async function seed() {
  for (const f of flowDefaults) await prisma.marketingResource.upsert({ where: { shop_kind_key: { shop: shop(), kind: "FLOW", key: f.key } }, create: { shop: shop(), kind: "FLOW", key: f.key, name: f.name, data: json({ trigger: f.trigger, description: f.description, reviewed: false, steps: f.delays.map((delay, index) => ({ minutes: delay, subject: index === 0 && f.key === "abandoned-cart" ? "Soft cart reminder" : f.name, channel: f.key === "low-stock" ? "SMS_TRANSACTIONAL" : "EMAIL", content: defaultContent })), ...(f.key === "abandoned-cart" ? { smsContent: { ...defaultContent, heading: "Still thinking it over?", body: "Your cart is waiting for you at Corals Anonymous.", button: "Complete your order" }, orderBranch: { yes: { subject: "Another Soft Push", content: { ...defaultContent, heading: "Your corals are still waiting", body: "Your order is not complete yet, but your cart is still saved.", button: "Return to your cart" } }, no: { subject: "Discount Offer", content: { ...defaultContent, heading: "A little something for your cart", body: "Complete your order and enjoy 10% off your corals.", button: "Claim your offer" } } } } : {}) }) }, update: {} });

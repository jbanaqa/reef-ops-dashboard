...
export async function identify(
  ...
  const identity = {
    email: input.email ? email(input.email) : undefined,
    phone: input.phone ? phone(input.phone) : undefined,
    shopifyId: input.shopifyId
      ? String(input.shopifyId).replace(/^gid:\/\/shopify\/Customer\//, "")
      : undefined,
  };
  const keys = Object.entries(identity)
    .filter(([, v]) => v)
    .map(([k, v]) => ({ [k]: v }));
  if (!keys.length)
    throw new Error("A verified customer identity is required.");
...
export async function record(
  ...
  return tx.marketingEvent.upsert({
    where: { shop_key: { shop: shop(), key: input.key } },
    create: {
      ...input,
      shop: shop(),
...
export async function consent(
  ...
  const current = await tx.marketingConsent.findUnique({
  ...
  await tx.marketingConsent.upsert({
    where: { profileId_channel: { profileId, channel } },
    create: { profileId, channel, ...data },
    update: data,
  });
  if (suppress)
    await tx.marketingMessage.updateMany({
      where: { profileId, channel, status: "PENDING" },
      data: { status: "CANCELLED", error: "Channel suppressed" },
    });
}
export function audienceWhere(
  ...
  return {
    ...
    ...(audience.openedDays
      ? {
          lastOpenedAt: {
            gte: new Date(+now - audience.openedDays * DAY),
            lte: now,
          },
        }
      : {}),
    ...(audience.tag ? { tags: { has: audience.tag } } : {}),
    ...(audience.list ? { lists: { has: audience.list } } : {}),
    ...(audience.purchasedDays
      ? {
          lastOrderAt: {
            gte: new Date(+now - audience.purchasedDays * DAY),
            lte: now,
          },
        }
      : {}),
    ...(audience.excludePurchasedDays
      ? {
          OR: [
            { lastOrderAt: null },
            {
              lastOrderAt: {
                lt: new Date(+now - audience.excludePurchasedDays * DAY),
              },
            },
          ],
        }
...
const b2bContent = {
  ...
  body: [
    ...
    "",
    "We are thrilled to welcome you to the Corals Anonymous Wholesale family!",
    "",
    "Here is what you can look forward to as a Corals Anonymous Wholesale customer:",
    "",
    "- ROTATING SUPER SALES EVERY WEEK with new items added daily. Ensure to keep an eye out for our newest arrivals!",
    "- WYSIWYG WEEKENDS, where selected WYSIWYG items are 50-80% OFF, every Friday at 7 AM PST.",
    "- 10% OFF COUPON CODE for your first order by signing up for our newsletter located at our homepage left bottom corner.",
    "",
    "Get started by logging into your wholesale account. Click the button below to visit our homepage and sign in. Please reach out if you have any questions!",
    "",
    "Best regards,",
    "Corals Anonymous Team",
  ].join(String.fromCharCode(10)),
  button: "Sign In & Buy Now!",
  url: "https://coralsanonymous.com",
  preview: "Welcome to Corals Anonymous Wholesale!",
  template: "b2b-wholesale",
};
export async function seed() {
  for (const f of flowDefaults)
    await prisma.marketingResource.upsert({
      where: { shop_kind_key: { shop: shop(), kind: "FLOW", key: f.key } },
      create: {
        shop: shop(),
        kind: "FLOW",
        key: f.key,
          ...
          ...(f.key === "abandoned-cart"
            ? {
                smsContent: {
                  ...
                  body: "Your cart is waiting for you at Corals Anonymous.",
                  button: "Complete your order",
                },
                orderBranch: {
                  yes: {
                    subject: "Another Soft Push",
                    content: {
                      ...defaultContent,
                      heading: "Your corals are still waiting",
                      body: "Your order is not complete yet, but your cart is still saved.",
                  ...
                  no: {
                    ...
                    content: {
                      ...defaultContent,
                      heading: "A little something for your cart",
                      body: "Complete your order and enjoy 10% off your corals.",
  ...
  await prisma.marketingResource.upsert({
    where: {
      shop_kind_key: { shop: shop(), kind: "TEMPLATE", key: "corals-standard" },
    },
    create: {
      shop: shop(),
      kind: "TEMPLATE",
      key: "corals-standard",
      name: "Corals Anonymous standard",
      data: json(defaultContent),
    },
    update: {},
  });
}

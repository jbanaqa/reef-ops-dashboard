...
export default function CollectionAutomationPanel({
  ...
  const loadAutomationData = useCallback(async () => {
    ...
    try {
      const [statusResponse, collectionsResponse] = await Promise.all([
        fetch("/api/collection-rotation/automation", {
          cache: "no-store",
        }),

        fetch("/api/collection-rotation/collections", {
          cache: "no-store",
        }),
      ]);

      const statusData = await readResponse<
        {
          ok: true;
        } & AutomationStatus
      >(statusResponse);

      const collectionsData = await readResponse<{
        ok: true;
      ...
      setRemainingMilliseconds(
        new Date(statusData.nextScheduledRunAt).getTime() - Date.now(),
      );
    } catch (error) {
      setErrorMessage(
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const remaining =
        new Date(status.nextScheduledRunAt).getTime() - Date.now();
  ...
  async function toggleAutomation(collection: CollectionSummary) {
    ...
    const nextEnabled = !collection.isEnabled;
    ...
    try {
      const response = await fetch("/api/collection-rotation/automation", {
        method: "POST",
      ...
      setStatus((currentStatus) =>
        currentStatus
          ? {
              ...
              enabledCollectionCount: data.enabledCollectionCount,
            }
          : currentStatus,
      );
  ...
  return (
    <section className="rotation-automation-card">
      <div className="rotation-automation-top">
        <div>
          <p className="page-header-eyebrow">Automatic collection rotation</p>
      ...
      <div className="rotation-countdown">
        {isLoading ? "--:--:--" : formatCountdown(remainingMilliseconds)}
      </div>
      ...
      {status ? (
        <>
          <p className="rotation-countdown-date">
            Scheduled for{" "}
            <strong>{formatDateTime(status.nextScheduledRunAt)}</strong>
          </p>

          <div className="rotation-automation-metrics">
            <div>
              <span>Schedule</span>
              <strong>{status.intervalLabel}</strong>
            </div>

            <div>
              <span>Enabled collections</span>
              <strong>{status.enabledCollectionCount}</strong>
            </div>

            <div>
              <span>Last scheduled cycle</span>

              <strong>
                {status.lastRun ? status.lastRun.status : "No runs yet"}
              </strong>
            </div>
          </div>
      ...
      {isManaging ? (
        <div className="rotation-automation-manager">
          <div className="rotation-automation-manager-header">
            <div>
              <h3>Automatic collections</h3>

              <p>
                Enabled collections rotate at each scheduled run using their
                saved strategy and fixed positions.
              </p>
            </div>
          ...
          <div className="rotation-automation-list">
            {visibleCollections.map((collection) => (
              <div key={collection.id} className="rotation-automation-row">
                <div className="rotation-automation-row-main">
                  <div>
                    ...
                    <p className="rotation-automation-row-meta">
                      ...
                      {strategyLabel(collection.strategy)} strategy
                      {collection.controlledTopCount > 0
                ...
                <label className="rotation-toggle">
                  <input
                    type="checkbox"
                    aria-label={"Automatic rotation for " + collection.title}
                    checked={collection.isEnabled}
                    onChange={() => void toggleAutomation(collection)}
      ...
      <p className="rotation-automation-note">
        You can also rotate collections from the collection library. Manual
        rotations do not reset or delay the automatic schedule.
      </p>
    </section>
  );
}

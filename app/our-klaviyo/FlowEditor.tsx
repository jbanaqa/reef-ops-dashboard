...
import { Content, content as normalizeContent, defaultContent, escapeHtml, MarketingSettings, render } from "@/lib/marketing/rules";
...
const startingHtml = (c: Content) => c.bodyHtml !== undefined ? c.bodyHtml : c.body.split(String.fromCharCode(10)).slice(c.template === "b2b-wholesale" ? 2 : 0).map(line => line ? `<p>${escapeHtml(line)}</p>` : "").join("\n");
...
export default function FlowEditor({ resource, busy, save, settings }: { resource: Resource; busy: boolean; save: (data: Data, enabled: boolean) => void; settings: MarketingSettings }) {
  ...
  const fields = (c: Content, target: Target) => <>
    ...
    <label key="html-copy-editor">Message HTML <small>Messages use HTML so you can control paragraphs, emphasis, colors, links, and lists. Use tags such as &lt;p&gt;, &lt;strong&gt;, &lt;em&gt;, and &lt;a href=&quot;https://...&quot;&gt;. Scripts and unsafe links are removed automatically.</small><textarea rows={10} value={startingHtml(c)} onChange={e => updateContent(target, "bodyHtml", e.target.value)} /></label>
  ...
  const imageFields = (c: Content, target: Target) => c.template !== "b2b-wholesale" ? null : <div className="mk-image-fields"><p className="mk-modal-label">Brand artwork</p><p className="mk-modal-help">The logo fills the white header. The footer image is the Klaviyo artwork shown in the blue footer (“Thank you for your business” and the heart). Uploading it replaces the placeholder text.</p><label>Logo image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={e => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = () => updateContent(target, "logo", String(reader.result)); reader.readAsDataURL(file); } }} /></label>{c.logo && <button type="button" className="mk-remove-image" onClick={() => updateContent(target, "logo", undefined)}>Remove logo</button>}<label className="mk-scale-control">Logo scale <span>{(c.logoScale || 1).toFixed(1)}×</span><input type="range" min="0.25" max="2.5" step="0.1" value={c.logoScale || 1} onChange={e => updateContent(target, "logoScale", Number(e.target.value))} /></label><label>Footer image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={e => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = () => updateContent(target, "footerImage", String(reader.result)); reader.readAsDataURL(file); } }} /></label>{c.footerImage && <button type="button" className="mk-remove-image" onClick={() => updateContent(target, "footerImage", undefined)}>Remove footer artwork</button>}<label className="mk-scale-control">Footer artwork scale <span>{(c.footerScale || 1).toFixed(1)}×</span><input type="range" min="0.25" max="2.5" step="0.1" value={c.footerScale || 1} onChange={e => updateContent(target, "footerScale", Number(e.target.value))} /></label></div>;

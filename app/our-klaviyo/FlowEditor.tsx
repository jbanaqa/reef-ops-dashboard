...
export default function FlowEditor({ resource, busy, save, settings }: { resource: Resource; busy: boolean; save: (data: Data, enabled: boolean) => void; settings: MarketingSettings }) {
  ...
  const fields = (c: Content, target: Target) => <>
    ...
    {c.bodyHtml !== undefined ? <label>Message HTML <small>Use simple email tags such as &lt;p&gt;, &lt;strong&gt;, &lt;em&gt;, &lt;a href=&quot;https://...&quot;&gt;, and lists. Scripts and unsafe links are removed automatically.</small><textarea rows={10} value={c.bodyHtml} onChange={e => updateContent(target, "bodyHtml", e.target.value)} /></label> : <label>Message<textarea rows={6} value={c.body || ""} onChange={e => updateContent(target, "body", e.target.value)} /></label>}
  ...
  const imageFields = (c: Content, target: Target) => c.template !== "b2b-wholesale" ? null : <div className="mk-image-fields"><p className="mk-modal-label">Brand artwork</p><p className="mk-modal-help">The logo fills the white header. The footer image is the Klaviyo artwork shown in the blue footer (“Thank you for your business” and the heart). Uploading it replaces the placeholder text.</p><label>Logo image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={e => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = () => updateContent(target, "logo", String(reader.result)); reader.readAsDataURL(file); } }} /></label><div className="mk-two"><label>Logo width (px)<input type="number" min="100" max="560" value={c.logoWidth || 260} onChange={e => updateContent(target, "logoWidth", Number(e.target.value))} /></label><label>Logo height (px)<input type="number" min="30" max="240" value={c.logoHeight || 78} onChange={e => updateContent(target, "logoHeight", Number(e.target.value))} /></label></div><label>Footer image<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={e => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = () => updateContent(target, "footerImage", String(reader.result)); reader.readAsDataURL(file); } }} /></label><div className="mk-two"><label>Footer width (px)<input type="number" min="120" max="600" value={c.footerWidth || 560} onChange={e => updateContent(target, "footerWidth", Number(e.target.value))} /></label><label>Footer height (px)<input type="number" min="30" max="260" value={c.footerHeight || 115} onChange={e => updateContent(target, "footerHeight", Number(e.target.value))} /></label></div></div>;

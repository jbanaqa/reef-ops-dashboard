  ...
  try {
    ...
    if (existingMapping) {
      return NextResponse.json(
        {
          error:
            "A mapping already exists for this RVS supplier item.",
        },
        {
          status: 409,
        },
    ...
    const mapping = await prisma.reorderMapping.create({
      data: {
        ...
        variants: {
          create: input.variants!.map((variant) => ({
            ...
            productId: cleanOptionalString(variant.productId),
    ...
    console.error("Failed to create reorder mapping:", error);

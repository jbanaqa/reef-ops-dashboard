...
export async function POST(request: Request) {
  ...
  try {
    ...
    const bucket = crypto
      .createHash("sha256")
      .update(
        `${request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown"}:${Math.floor(Date.now() / 3600000)}`,
      )
      .digest("hex");
    await atomic(async (tx) => {
      const key = `rate:${bucket}`;
      const r = await tx.marketingResource.findUnique({
        where: { shop_kind_key: { shop: shop(), kind: "RATE", key } },
      });
      ...
      await tx.marketingResource.upsert({
        ...
        create: {
          ...
          kind: "RATE",
          key,
          name: "Storefront rate limit",
          data: { count: 1 },
        },
        update: { data: { count: count + 1 } },
      });
    });
    if (b.action === "signup") {
      const welcome = await prisma.marketingResource.findUnique({
      ...
      if (b.emailConsent !== true)
        throw new Error("Email marketing consent is required.");
      ...
      await atomic(async (tx) => {
        ...
        await record(tx, {
          key: `form:${session}`,
          type: "FORM_EMAIL_SUBMITTED",
          profileId: p.id,
          payload: {
            disclosure: "Email marketing; 10% first order offer",
            version: "v1",
          },
        });
        ...
        await tx.marketingResource.create({
          data: {
            ...
            kind: "SIGNUP",
            key: session,
            name: "Pending email confirmation",
            data: {
              profileId: p.id,
        ...
        await tx.marketingMessage.create({
          data: {
            ...
            flowKey: "email-confirmation",
            channel: "EMAIL",
            subject: "Confirm your Corals Anonymous email signup",
            content: json({
              ...defaultContent,
              heading: "Confirm your email",
              body: "You requested email updates and a first-order offer. Confirm your email to complete signup. If you did not request this, ignore this message.",
    ...
    if (b.action === "sms") {
      if (!config.smsReady)
        throw new Error("Text signup is not available yet.");
      if (b.smsConsent !== true)
        throw new Error("Explicit SMS marketing consent is required.");
      ...
      await atomic(async (tx) => {
        const session = await tx.marketingResource.findUnique({
          where: {
            shop_kind_key: {
              shop: shop(),
              kind: "SIGNUP",
              key: String(b.session),
            },
          },
        });
        const d = session?.data as
          | { profileId: string; confirmed: boolean; expiresAt: string }
          | undefined;
        if (!d?.confirmed || new Date(d.expiresAt) < new Date())
          throw new Error(
            "Confirm your email first, then return here to add SMS.",
          );
        ...
        await consent(
          ...
          "SMS_MARKETING",
          "SUBSCRIBED",
          "storefront-explicit-sms-v1",
          new Date(),
        );
        await record(tx, {
          key: `form-sms:${session!.key}`,
          type: "FORM_SMS_SUBMITTED",
          profileId: p.id,
          payload: {
            version: "v1",
            disclosure:
              "Recurring automated marketing texts; consent not a condition of purchase; message/data rates may apply; STOP to cancel.",
          },
        });
      });
    ...
    if (b.action === "event") {
      if (
        ...
        throw new Error("Unsupported storefront event.");
      if (!/^[a-zA-Z0-9_-]{10,100}$/.test(b.anonymousId) || !b.id)
        throw new Error("Anonymous event identity required.");
      // Untrusted browser events remain anonymous and cannot assert purchases, consent or Shopify IDs.
      await atomic((tx) =>
        record(tx, {
          key: `browser:${String(b.id).slice(0, 100)}`,
          type: b.type,
          anonymousId: b.anonymousId,
          payload: {
            productId: String(b.productId || "").slice(0, 100),

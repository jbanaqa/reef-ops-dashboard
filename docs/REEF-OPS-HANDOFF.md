...
### Branded unsubscribe and Shopify opt-out sync — September 23, 2026

Customer email unsubscribe POSTs now suppress Reef Ops immediately and durably queue a matching Shopify customer email-consent update. The request tries Shopify immediately; the marketing worker retries failures with backoff even while sending is paused. Shopify lookup requires one exact email match, and success requires Shopify to return `UNSUBSCRIBED`. Repeated POSTs are safe. SMS unsubscribe behavior remains local and does not alter Shopify email consent. The customer page has no dashboard navigation, uses no external assets, and has no-referrer/no-index headers. `MARKETING_UNSUBSCRIBE_ORIGIN` can point all future customer email links and List-Unsubscribe headers to a branded HTTPS hostname; the proxy serves only `/api/marketing/unsubscribe` on that hostname and returns 404 for all other routes. Existing links on the app hostname continue working.

Production setup remains incomplete: the installed Shopify app has `read_customers` but lacks the `write_customers` scope required by `customerEmailMarketingConsentUpdate`. Railway has reserved `unsubscribe.coralsanonymous.com`, but its DNS CNAME `unsubscribe` → `b65fczdq.up.railway.app` is not yet published, so do **not** set `MARKETING_UNSUBSCRIBE_ORIGIN` until Railway reports a valid certificate and a live HTTPS check passes. `email.coralsanonymous.com` is already a SendGrid CNAME and was deliberately left untouched. Once the scope and DNS are ready, set `MARKETING_UNSUBSCRIBE_ORIGIN=https://unsubscribe.coralsanonymous.com`, test the branded URL and host isolation, and reconcile the one pre-existing Reef Ops unsubscribe-link opt-out with Shopify. No customer consent, current sending restriction, or live email was changed while preparing this code.

### Final Welcome social email — September 23, 2026

The fourth Welcome Series email now follows the supplied Klaviyo social-email screenshot: shared large logo, three editable navigation links, Instagram gradient and Facebook blue panels with editable card copy and handles, a divider, and six products in a three-column desktop grid. The shared blue “Thank you for your business” footer remains intact. The panels use hosted PNG assets for reliable email rendering. The social grid uses the same validated Shopify product-feed selector as campaigns, defaulting to the six newest eligible products; feed, sort, count, product preview, image width, and button styling are editable in Layout. At delivery preparation, the latest Shopify products replace editor placeholders and preparation fails for retry if the feed is unavailable or empty. Saved Welcome social copy survives the layout upgrade. The grid stacks on mobile and was visually checked at 600px and 375px with no horizontal overflow. The marketing test suite (100 tests), TypeScript, targeted ESLint, and production build check pass. No live email was sent.

### Welcome reminder emails — September 23, 2026

The second and third Welcome Series emails now use a shared 500×200 crisp clownfish/coral banner (`public/welcome-reminder-banner.png`) modeled on the supplied Klaviyo screenshots, with live editable message text over the blank speech bubble. Their coupon heading, code pill, orange CTA, and compact spacing match the references; the third email adds an orange expiry line. `{{ coupon_time_left }}` resolves to the real assigned coupon's remaining days at preparation, so the visible wording cannot claim a fixed seven days when the actual expiration differs. After review, the generated gray image placeholder was removed; an image area appears only when staff explicitly upload an optional reminder image in Artwork. The shared logo now appears above all three illustrated Welcome emails, and the reminder bubble text is centered within the white space at desktop and mobile sizes. Saved customized body copy and artwork are preserved by the Welcome draft upgrade. The shared blue “Thank you for your business” footer remains unchanged, as do scheduling, coupon assignment, eligibility, and send guards.
...
The B2B-style layout's formerly implicit first-line greeting is now an editable **Intro line**. Legacy B2B emails retain their greeting, while the 24-hour notice explicitly leaves this field blank so its shipping sentence appears only once from Message. Existing saved 24-hour defaults are corrected in the editor and at delivery validation without overwriting custom copy.

A follow-up audit rendered all ten saved flow email paths: the 24-hour notice, four Welcome messages, the initial and both branched Abandoned Cart messages, B2B Welcome, and Low Stock. No message-copy phrase appears more than once. The formerly hardcoded footer **Follow Us** label is now an editable Social heading, every layout's default unsubscribe introduction is visible in Footer and may be cleared, and the Standard header uses the editable organization name from Settings. Empty editable labels stay empty instead of silently reverting to generated wording. Missing product artwork is now a blank visual placeholder, so product names appear only once. Automated coverage renders Standard, B2B, Cart Recovery, Welcome, Welcome Social, and Campaign Sale layouts with customized and cleared controls.

Footer background color is a universal per-email control in the shared Footer tab. Existing layouts retain their prior defaults until changed: Standard white, B2B/24-hour navy, Cart Recovery teal, Welcome light gray, and Campaign Sale black. The saved color drives editor previews, test emails, flow delivery, templates, and campaigns; staff may restore the current layout's default with one button.

The shared Footer tab also has a **Show social media in footer** control. It defaults on to preserve existing emails. Turning it off hides the social heading, links, and icons in previews, tests, and delivered messages while retaining the saved social content so it returns unchanged if re-enabled. This footer control does not remove a Welcome Social email's main social-content section.
...
Reef Ops also subscribes to Shopify `orders/updated`. A recognized delivery-date change reschedules the existing pending notice for that order instead of creating another run. Removing the date, changing it to an invalid value, or moving it past the configured reminder window cancels the pending notice. A later valid future date restores that same notice. Messages already sent or currently being delivered are never duplicated or recalled.
...
### Welcome first-email visual parity — September 23, 2026

The first Welcome Series email now uses a locally hosted clownfish/coral speech-bubble illustration (`public/welcome-hero-crisp.png`) with live editable greeting and message over it. The crisp replacement was derived from the original Klaviyo popup artwork (`public/welcome-popup-art.png`) and the supplied email screenshot; the earlier blurry recreation was removed and remains only in Git history. The illustration is optional in Layout; a custom Hero image still replaces it. The coupon, body emphasis, and spacing follow the supplied Klaviyo screenshots more closely, while the existing shared blue “Thank you for your business” footer remains under the message. The coupon assignment, timing, expiration, and delivery rules are unchanged. Legacy untouched welcome copy gains editable rich markup in the editor and at render time; customized saved copy is preserved. The illustrated layout suppresses only the default fine-print line to match the reference; explicitly customized coupon terms or expiry text remain visible. Desktop/375px/320px renders have no horizontal overflow. Verification: 98 marketing tests, TypeScript, targeted ESLint, and production build pass.

...
Shopify: preserve the existing orders/create webhook and enable MARKETING_INGEST_ENABLED after migration. Register customers/create, customers/update and checkouts/create to `/api/marketing/webhooks?source=shopify`; register orders/create there only if the existing order webhook is not in use. Enable required customer/order access in the app. No remote subscriptions are created by this implementation. Import the initial customer base; webhooks cover subsequent changes. Checkouts without usable customer identity cannot enroll and require upstream reconciliation. Order cancellation checks depend on timely successful ingestion, so validate this before enabling abandoned cart.
